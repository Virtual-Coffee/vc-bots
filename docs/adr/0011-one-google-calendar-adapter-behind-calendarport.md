...
- **Announcements reach the Calendar through `EventSource` (`src/bots/reminders/source.ts`)**:
  a registry of named sources — Google, and the interim `cms` source that is the `EVENT_SOURCE`
  default until the cutover ([0001](0001-event-model-join-link-and-host-key.md)). Cron
