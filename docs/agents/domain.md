...
Note that `CLAUDE.md` at the repo root already carries the architectural orientation (request flow, invariants, the co-working Durable Object, reminders, conventions). Read it alongside these; `CONTEXT.md` is for domain vocabulary, not a duplicate of it.
...
> _Contradicts ADR-0003 (Zoom events serialized in the DO), but worth reopening because…_
