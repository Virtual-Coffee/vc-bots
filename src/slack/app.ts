...
export function createSlackApp(env: Env, publicBaseUrl: string): SlackApp<Env> {
  return (
    new SlackApp<Env>({
      ...
      .command(ADMIN_COMMAND, ack, async ({ payload }) => handleAdminCommand(payload, env))
      ...
      .viewSubmission(
        REMINDER_MODAL_CALLBACK_ID,
        async () => {},
        async ({ payload }) => handleReminderSubmit(payload, env),
      )
      .viewSubmission(
        WELCOME_MODAL_CALLBACK_ID,
        async () => {},
        async ({ payload }) => handleWelcomeSubmit(payload, env),
      )
      .viewSubmission(
        COWORKING_MODAL_CALLBACK_ID,
        async () => {},
        async ({ payload }) => handleCoworkingSubmit(payload, env),
      )
  );
}
